...
- Dropdown focus guard validation and E2E passed and must stay part of menu accessibility validation; dropdown/menu hiding now releases any focused descendant to its menu trigger before `aria-hidden=true` is applied, preventing Chromium "Blocked aria-hidden" console warnings without changing menu commands or solver behavior.
...
- PDF export progress validation and E2E passed and must stay part of Menu -> File -> Export PDF validation; PDF export now shows a compact professional progress overlay from 5% to 100%, reports active simulation read/topology/model snapshot/phase chart/Moody chart/equation/page/finalization milestones, disables only PDF export commands while active, restores UI on success/failure, leaves XLSX export untouched, and verifies the generated report still contains Equation Professional, Pressure-enthalpy phase chart, compact Log-Log Moody Chart, and no Pump Performance Curve section.
- Excel calculation trace validation, Excel COM sensitivity validation, and E2E passed and must stay part of Menu -> File -> Export Excel Calculation Trace validation; XLSX export is separated from PDF report rules and now produces a compact formula-driven workbook with one editable Inputs sheet plus formula-linked OUTPUT PUMP and OUTPUT PFV dashboard blocks matching the requested reference layout, application-resolved engineering reference-baseline parity plus cached application-result values so default NPSHa and related pump values match the app immediately, no non-engineering Trace control/toggle rows, live Fluid Basis property calculations, Water-only Pressure-Enthalpy phase data/chart sheets that are completely omitted for Methanol, Custom Fluid, and every other non-Water basis, suction and discharge Pipe/Fitting/Valve calculations, NPSHa/NPSHr margin/ratio, discharge pressure, required head, and native OpenXML P-h/Moody charts including Pipe 2 when a discharge pipe exists; the sensitivity validator opens Excel, changes 14 major input parameters, recalculates, and verifies dependent outputs change.
