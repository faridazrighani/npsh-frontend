...
const CACHE_KEY = "engineering-pipe-canvas-hydraulic-label-runtime-20260716-smooth-drag1.js?v=20260716-canvas-object-smooth-drag1";
...
assert(runtime.includes("canvasDrawFrame"), "Runtime must coalesce canvas redraws to one animation frame while dragging.");
assert(runtime.includes("canvasDrawPending"), "Runtime must retain only the latest pending drag redraw.");
assert(runtime.includes('typeof root.requestAnimationFrame === "function"'), "Runtime must use the browser animation frame for smooth canvas dragging.");
assert(runtime.includes("queueCanvasGeometryDraw"), "Runtime must share one animation-frame geometry queue across nested canvas wrappers.");
assert(runtime.includes("pipeCanvasHydraulicDragGate"), "Runtime must install an outer drag gate above existing canvas wrappers.");
assert(runtime.includes("__pipeCanvasHydraulicDragGateOriginal"), "Runtime drag gate must retain the wrapped canvas chain for non-drag redraws.");
assert(runtime.includes("Object.getOwnPropertyNames(candidate)"), "Runtime must inspect nested wrapper chains before installing another full draw patch.");
assert(runtime.includes("if (hasPipeCanvasDrawPatch(original))"), "Runtime must place a lightweight gate above an existing hydraulic-label draw patch.");
assert(runtime.includes("refreshConnectionGeometryDuringDrag"), "Runtime must update existing pipe path geometry without rebuilding the full SVG while dragging.");
assert(runtime.includes('svg.querySelectorAll(".pipe-line[data-pipe-id]")'), "Runtime drag fast path must target the existing hydraulic route paths.");
assert(runtime.includes('path.setAttribute("d", pathData(points))'), "Runtime drag fast path must update only pipe path geometry.");
assert(runtime.includes("scheduleFullCanvasDrawAfterInteraction"), "Runtime must perform one complete redraw after canvas dragging settles.");
assert(runtime.includes("deferRefreshUntilInteractionSettles();"), "Runtime must defer PFV label work until canvas dragging settles.");
assert(runtime.includes("if (isCanvasInteractionActive()) deferRefreshUntilInteractionSettles();"), "Mutation observer must not force PFV repaint while an object is being dragged.");
