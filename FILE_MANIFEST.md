...
engineering-simulation-load-transaction-manager.js public-safe Simulation Load Transaction Manager for Menu -> File -> Open and Simulation Case opens; creates a per-load session token, aborts stale fetch/body/file reads with AbortController/WeakMap guards, warms key runtime scripts and selected simulation-case payloads in browser cache, cleans transient warning/route-trace UI before applying the next model, tags backend calls with X-NPSH-Load-Session, releases Validate/Run command locks on load complete/abort/failure, and prevents stale file/case results from leaking into the next active simulation without changing solver formulas
...
engineering-pipe-properties-cleanup-runtime.js public-safe early Pipe Properties hard cleanup and stability guard that removes unsupported top-level Pipe Properties fields before visible paint/repaint, removes segment z columns and optional hL Allow column, hooks calculation refresh mutations, and preserves Pipe Segment horizontal/body scroll during user input
...
engineering-pipe-canvas-hydraulic-label-runtime.js public-safe compact Pipe/Fitting/Valve canvas label bridge that suppresses the old delta-P-only SVG label and replaces it with live velocity, Total K, Total hL, Minor loss, and Major loss readouts from current pipe results/calculation trace; Total K is formatted to 3 decimals, hydraulic head/velocity values remain formatted to 5 decimals, and pressure readouts stay in Pipe Formula Defense, with pump-label-sized upright canonical canvas-anchor placement that stays in the same pipe-relative position on desktop and cellular viewports, and synchronous no-flicker refresh after solver/canvas redraws, without changing calculation data
...
engineering-canvas-clear-reset-guard.js public-safe global Clear Canvas and Reset Canvas View guard that suppresses live-panel resurrection during canvas clear, removes transient pump/source/sink/PFV/route overlays while preserving the Fluid Basis dock, resets warning/context-menu state, and restores canvas scroll/anchor origin after menu reset
...
Export equation professional runtime cache key: engineering-export-equation-professional-runtime.js?v=20260707-pdf-equation-professional9
...
Analysis Report live runtime cache key: engineering-analysis-report-live-runtime.js?v=20260706-status-matrix-lock1
...
Simulation load transaction manager cache key: engineering-simulation-load-transaction-manager.js?v=20260707-simulation-load-transaction6
...
Suction-only NPSHa runtime cache key: engineering-suction-only-npsha-runtime.js?v=20260707-suction-only-stale-discharge1
Calculation lifecycle runtime cache key: engineering-calculation-lifecycle-runtime.js?v=20260707-solver-release-watchdog3
...
Open file readiness gate cache key: engineering-open-file-readiness-gate.js?v=20260707-open-file-readiness-gate9
...
Canvas clear/reset guard cache key: engineering-canvas-clear-reset-guard.js?v=20260707-clear-keeps-fluid-basis1
