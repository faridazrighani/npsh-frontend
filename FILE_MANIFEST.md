...
engineering-pump-performance-canonical-chart.js public-safe operational smart engineering chart/runtime guard that keeps established pump NPSH chart-model refresh logic available for dependencies, suppresses pump-development UI entry points (Pump Object Properties and Pump Performance Chart task windows), exposes Manual NPSHr and Pump Formula Defense task windows from the pump context menu, safely releases context-menu focus before aria-hidden, and schedules one governed visible-chart render so dependent chart numbers can refresh without repeated repaint bursts
