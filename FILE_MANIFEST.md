...
engineering-pipe-canvas-hydraulic-label-runtime.js public-safe compact Pipe/Fitting/Valve canvas label bridge that suppresses the old delta-P-only SVG label and replaces it with live static endpoint pressure, velocity, total K, major loss, and minor loss readouts formatted to 3 decimals from current pipe results/calculation trace, with pump-label-sized upright canonical canvas-anchor placement that stays in the same pipe-relative position on desktop and cellular viewports, and synchronous no-flicker refresh after solver/canvas redraws, without changing calculation data
