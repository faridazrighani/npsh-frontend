...
tools/validate-simulation-load-transaction-manager.cjs Node validation for Simulation Load Transaction Manager runtime loading, cache key, index load order after app.bundle and before Open File readiness, session-token API, AbortController cleanup, stale-result guard strings, npm script, and manifest lock
