...
tests/                  Playwright browser E2E coverage for realtime SINK backend recalculation, Moody/defense gates, Pipe Segments local file import/export, Formula Defense UI visibility/rendering, live model-linked Pipe Formula Defense numeric refresh, dropdown focus/aria-hidden guard verification, PDF export progress/report-content verification, Excel calculation trace workbook export verification, and Pump Performance Chart backend-refresh fingerprints
...
engineering-pump-status-visual-lock.js public-safe Pump canvas status-badge guard that keeps freshly dragged, disconnected, or hydraulically incomplete pumps visually Incomplete and prevents stale Safe fields from overriding hydraulicNpshStatus
engineering-pump-envelope-warning-cleanup-runtime.js public-safe warning lifecycle guard that sanitizes deprecated pump performance/envelope/BEP/design-duty warnings and expires obsolete backend-unavailable warnings from completed, superseded, calculating, or replaced requests while preserving active backend failures, hydraulic NPSH warnings, cavitation warnings, and Manual NPSHr validation
...
engineering-sink-input-stability-runtime.js SNK Object Properties numeric-input fast lane that keeps Flow Demand, Reference Pressure, and Elevation user-owned while typing, protects recent input preview from unrelated calculation-current repaint pulses until the debounce window settles, coalesces keystrokes into one protected backend transaction, and prevents competing legacy listeners from replacing a partially typed value
...
engineering-route-trace-audit.js public-safe backend route-trace audit bridge, dependency fingerprint handoff, stale-result metadata capture, fixed pump/SNK presentation rows, canonical Hydraulic NPSH/Backend Valid status normalization, a five-row SNK canvas template, and one orientation-independent route presentation matrix that propagates risk/warning/incomplete/safe colors across SRC, suction PFV, Pump, discharge PFV, and SNK for both forward and reverse construction order without changing hydraulic results
engineering-route-trace-audit-20260704-sink-pabs-dedupe1.js cache-busted production alias for the route-trace audit runtime, used to bypass stale custom-domain asset cache after the Sink P abs duplicate-row cleanup
...
engineering-simulation-load-transaction-manager.js public-safe Simulation Load Transaction Manager for Menu -> File -> Open and Simulation Case opens; creates a per-load session token with transaction-owned AbortController signal, aborts stale fetch/body/file reads with AbortController/WeakMap guards, absorbs superseded openSimulationCaseSample promises as audited stale results instead of uncaught AbortError rejections, captures and awaits the calculation Promise started by project hydration, verifies primaryApplied evidence, and invokes the protected backend runner directly only when a wrapper returned without performing the authoritative calculation; exposes assertCurrent/guardAsyncResult/signal/setSessionTimeout APIs, warms key runtime scripts with pathname-based deduplication while leaving the critical open-file readiness gate under index.html ownership, warms selected simulation-case payloads, cleans transient warning/route-trace UI before applying the next model, tags backend calls with X-NPSH-Load-Session, releases Validate/Run and File Export locks after load settlement, and prevents saved or stale case results from becoming the ready state without changing solver formulas
engineering-simulation-load-transaction-manager-20260715-export-lock-dedupe1.js cache-busted production alias for the Simulation Load Transaction Manager export-lock and pathname-deduplication release, used to bypass stale custom-domain immutable asset cache
engineering-performance-baseline-runtime.js public-safe read-only performance baseline instrumentation for live/local reliability checks; records simulation load, apply-model, Validate/updateSimulation, calculation lifecycle, canvas-ready, console warning/error, stale-result, long-task, canvas DOM count, task-window count, warning count, and Validate stuck indicators through an in-memory ring buffer and event samples without calling backend/solver/export APIs or mutating hydraulic data; routine console output is opt-in and orphan Current pulses are counted without duplicate calculation-complete samples
engineering-runtime-health-audit.js public-safe runtime health audit, self-healing guard, idle maintenance layer, load-footprint trend guard, and opt-in reliability evidence snapshot panel for repeated File Open/Simulation Case load cycles; listens to settled-load and performance-baseline events, keeps bounded in-memory audit/load/footprint history, dedupes transaction/baseline settle pairs, detects stale Validate busy state, stale load-active class, stale visual refresh queues, DOM/task-window budget pressure, rapid load bursts, and post-load DOM/heap footprint growth, then safely delegates to Simulation Load Transaction Manager cleanup/watchdog APIs, prunes transient load artifacts, trims idle diagnostic buffers, and exposes a JSON evidence snapshot without calling solver/backend/export APIs or mutating hydraulic model data
...
engineering-pump-edit-fast-lane.js public-safe Pump Object Properties and compact Manual NPSHr task fast-lane runtime that classifies pump-only edits, updates Manual NPSHr, local NPSH margin/ratio/status with canonical OK/Warning/Cavitation Risk/NPSHr Not Provided labels and pump chart preview without changing Pump Properties layout, keeps Manual NPSHr from triggering backend autosolve or recalculating suction-side NPSHa, keeps route-only NPSH/margin edits from filling Actual Pump Head or power from design/required head, and still requests protected backend autosolve for pump fields whose connected-route NPSHa/status must be recalculated
engineering-realtime-calculation-defense.js public-safe realtime stale/calculating/freshness bridge that marks calculation results stale/calculating when object-property inputs or upstream dependency inputs change, exposes a global dependency-change bridge for Fluid Basis/SRC/pipe/sink route dependencies, covers persistent Object Properties surfaces including compact Manual NPSHr, autosolves trusted user edits and explicit dependency notifications through the protected backend as the primary calculation path with 240 ms input and 90 ms change debounce, carries sequence/request transaction metadata plus explicit realtime-input mode, records initial/final dependency fingerprints, emits superseded events without letting stale backend results overwrite newer input, heals realtime completion from current backend result/cache data so calculationId, dependencyFingerprint, and Backend Valid. cannot remain Calculating after a successful response, exposes Failed when backend recalculation fails while keeping the last valid result visible, suppresses bootstrap/sample-menu Calculating status until explicit user calculation intent, treats Simulation Case browsing as menu-only, publishes canonical pumpHead as actual-only with requiredSystemHead separate, avoids heavy chart/linked-view refresh during typing, debounces linked-view refresh through the Performance Refresh Governor after backend completion, and refreshes linked report/task-window views from current backend results
engineering-fluid-basis-phase-chart-runtime.js public-safe Fluid Basis pressure-enthalpy phase chart runtime that displays the clean default Water P-h chart only when Fluid Name is Water, removes it immediately for Methanol or Custom Fluid, restores it when Water is selected again, reads temperature and Vapor Pressure from Fluid Basis, draws proportional IAPWS/IF97-style saturation dome and temperature curves, never calls backend calculations, and exposes Water-only export markup for the PDF Fluid Basis discussion
engineering-source-volumetric-only-runtime.js public-safe Source Properties volumetric-only guard that locks SRC flow basis to Volumetric Flow, moves the editable Volumetric Flow field into Boundary Data, synchronizes connected SNK Flow Demand from the SRC Volumetric Flow input, removes Source Definition/Source Type/Type Meaning and Flow Specification/Flow Input Mode/Mass Flow task rows without visible Boundary Data layout flash during input or solver refresh, removes legacy SRC source-type context menu choices while keeping Object Properties/Connect/Delete Source, converts legacy Mass Flow mode sources into equivalent volumetric flow once, and keeps Mass Flow only as hidden derived data
engineering-src-connect-context-runtime.js public-safe SRC context menu lock that intercepts source object context menus before legacy source-type choices render, keeps only User Task Object Properties, Connect, and Delete Source, starts the native hydraulic Pipe/Fitting/Valve connection mode from SRC when Connect is clicked, and delegates Object Properties/Delete Source to the protected app commands
engineering-suction-only-npsha-runtime.js public-safe suction-only NPSHa bridge that detects Fluid Basis/SRC to suction PFV to Pump routes without downstream Sink, fingerprints suction-side inputs to avoid repeated refreshes, requests one protected backend calculation when the suction fingerprint changes, hydrates SRC/PFV/Pump readouts from the current suction-side inputs when protected results do not repaint the canvas, applies Suction Only, Downstream Required, and canonical NPSHr Not Provided/OK/Warning/Cavitation Risk status readouts, refreshes PFV canvas hydraulic labels, and keeps Required Pump Head, Discharge Pressure, Actual Pump Head, and pump power unavailable until a downstream route exists
engineering-calculation-lifecycle-runtime.js public-safe lifecycle status bridge that unifies input-changed, preparing, waiting-debounce, calculating, applying-results, refreshing-evidence, current, and failed states from realtime calculation events and Validate/Refresh/Open Sample Case commands, separates realtime-input/menu-browse/sample-open/manual-solve modes, treats the manual command as Validate / Refresh Evidence while realtime autosolve remains primary, keeps Run/Validate commands passive during realtime input autosolve, only disables Run/Validate for manual/sample/evidence work, suppresses bootstrap calculating/applying and orphan evidence-refresh events while sample-case selection is idle, permits Refreshing evidence only for manual evidence refresh, releases stuck Validate/Run busy states with command watchdogs, post-settle availability audits, and simulation-load settle events, and never mutates calculation data
engineering-calculation-progress-overlay.js public-safe non-blocking compact calculation progress overlay for manual Validate/Refresh/Open Sample Case/Simulation Case browse intent, suppresses overlay display for realtime input autosolve so input edits remain non-blocking, shows only Reading inputs for menu browsing, shows Reading/Solving/Updating for sample-open, reserves Refreshing evidence for manual evidence refresh, auto-hides on Current/fallback evidence completion, force-hides repeated evidence refresh loops and load-settle events, shows short failed-calculation fallback text for manual evidence runs, and never mutates model/results/traces/charts
engineering-open-file-readiness-gate.js public-safe blocking open-file readiness gate for Menu -> File -> Open .untirta workflow; hides the canvas while the selected file is being read, validated, hydraulically solved, canvas-updated, and display-finalized, directly loads the pipe-label and route-cleanup runtimes needed for readiness, accelerates deferred realtime/support runtimes, waits for quiet/clean canvas or sustained stable readiness evidence plus pipe/fitting/valve hydraulic parameter labels before release, throttles pipe-label refreshes and display cleanup so the readiness quiet-window can complete, runs final forced route-trace and pipe-label cleanup and then verifies post-cleanup pipe labels before release, disables export/solve commands during preparation, and never mutates model/results/calculation formulas
...
engineering-pipe-properties-cleanup-runtime.js public-safe early Pipe Properties hard cleanup and stability guard that removes unsupported top-level Pipe Properties fields before visible paint/repaint, removes segment z columns and optional hL Allow column, preserves Pipe Segment horizontal/body scroll during user input, and locks Pipe Fitting Valve Breakdown display precision to 3 decimals for Total K and 5 decimals for Major/Minor/Total hL
...
engineering-pipe-canvas-hydraulic-label-runtime.js public-safe compact Pipe/Fitting/Valve canvas label bridge that suppresses the old delta-P-only SVG label and replaces it with live velocity, Total K, Total hL, Minor loss, Major loss, Primary Reynolds, and Darcy f readouts from current pipe results/calculation trace; Reynolds and Darcy f use the same primary Moody marker and display rules as Moody Chart / Friction Factor Check, Total K is formatted to 3 decimals, hydraulic head/velocity values remain formatted to 5 decimals, and pressure readouts stay in Pipe Formula Defense, with pump-label-sized upright canonical canvas-anchor placement that stays in the same pipe-relative position on desktop and cellular viewports, retained SVG node identity across connection redraws, stable-value holding while calculation results are provisional, and synchronous no-flicker final refresh without changing calculation data
engineering-model-snapshot-export-runtime.js public-safe Menu -> File -> Export model snapshot capture guard that first captures the actual visible #canvas DOM workspace crop with Fluid Basis, route, objects, SVG lines, and Pipe/Fitting/Valve labels exactly as seen on screen, using content-bounds width/height from the rightmost and lowest visible model panel/label plus compact margins so the crop matches the provided example instead of exporting blank grid; composites the Fluid Basis dock as a deterministic compact canvas layer with route pills to avoid full-canvas foreignObject artifacts; keeps native/original canvas plus dock composition and compact trace overlay only as fallbacks; retains the readable Fluid Basis/route renderer as an explicit opt-in API for future use
engineering-export-equation-professional-runtime.js public-safe PDF-only Menu -> File -> Export appendix layout bridge that forces Calculation Trace PDF output into English professional mechanical/chemical engineering phrasing, compact Equation Professional sequential blocks with equation, numerical substitution, and evaluated result stacked together for every calculation step; every standalone equation line and sequential equation step carries Mode: Equation Professional with centered display-equation styling, refreshes/sanitizes export data against the active topology so reduced suction-only, incomplete, or reversed routes do not leak stale discharge/SNK report steps, removes unused Pump Performance Chart/Curve report content, adds Fluid Basis Pressure-enthalpy phase chart SVG discussion for liquid/mixed-phase/vapor visualization, replaces the old/native Moody chart image/table with a compact Log-Log Moody Chart friction-factor visual using fixed segment-card copy columns, leaves Excel on its original exporter for future separate rules, and keeps DOCX hidden from the menu
engineering-dropdown-focus-guard-runtime.js public-safe menu accessibility bridge that patches Element.setAttribute for dropdown/menu hiding so focused descendants are released to their menu trigger before aria-hidden=true is applied, preventing Chromium "Blocked aria-hidden" console warnings for Simulation/File/Tools dropdowns without changing menu commands or solver behavior
engineering-pdf-export-progress-runtime.js public-safe compact Menu -> File -> Export PDF progress overlay and event bridge that reports real milestone progress from 5% to 100%, including active simulation read, topology validation, model snapshot capture, Fluid Basis phase chart, per-pipe Moody chart evidence, Equation Professional formatting, PDF page build, and finalization; disables only PDF export commands during the active export, restores UI on success/failure, leaves XLSX export untouched, and never calls solver/backend/network APIs
engineering-excel-calculation-trace-runtime.js public-safe Menu -> File -> Export Excel Calculation Trace workbook override that creates a compact formula-driven XLSX with one editable Inputs sheet plus formula-linked OUTPUT PUMP and OUTPUT PFV dashboard blocks matching the requested reference layout, then Fluid Basis, suction PFV, discharge PFV, NPSH, pump discharge, and Calculation Sequence sheets; creates Pressure-Enthalpy data/chart sheets and the native P-h chart only when the active Fluid Basis is Water, while Methanol, Custom Fluid, and every non-Water basis omit both P-h sheets entirely; uses application-resolved engineering values as export reference baselines so exported NPSHa/NPSHr margin/ratio, suction/discharge loss, vapor pressure head, discharge pressure, and required head match the app at the initial case while remaining live when Temperature, SRC, SNK, NPSHr, pipe, fitting, and valve inputs change; caches those application results inside formula cells for immediate display parity in Excel/viewers; does not expose non-engineering Trace control/toggle rows; recalculates water density, kinematic viscosity, dynamic viscosity, vapor pressure, specific weight, vapor pressure head, specific gravity, specific volume, specific heat, bulk modulus, speed of sound, Darcy/Re/epsD/f/major/minor losses, NPSHa, manual NPSHr margin/ratio, discharge pressure, and required head through linked Excel formulas; injects native OpenXML scatter charts for Water P-h, suction Moody, and Pipe 2/discharge Moody when present without calling solver/backend APIs
engineering-excel-calculation-trace-runtime-20260715-water-only-ph-sheets1.js cache-busted production alias for the Water-only Excel Pressure-Enthalpy sheet release, used to bypass stale custom-domain immutable asset cache
engineering-canvas-fast-preview-runtime.js public-safe canvas fast-preview bridge that repaints Pump, Pipe/Fitting/Valve, and SNK live readouts on the next animation frame after input/dependency changes, keeps preview values outside persistent pump results, atomically synchronizes NPSHa aliases from the latest authoritative solve, cancels stale preview work on calculation completion and workspace load, normalizes transient status labels, and does not call updateSimulation
...
engineering-canvas-clear-reset-guard.js public-safe global Clear Canvas and Reset Canvas View guard that turns File/Edit Clear Canvas into a browser clean reload for workspace reset, suppresses live-panel resurrection during legacy/internal canvas clear, removes transient pump/source/sink/PFV/route overlays while preserving the Fluid Basis dock, resets warning/context-menu state, and restores canvas scroll/anchor origin after menu reset
...
engineering-pipe-moody-chart-audit.js public-safe pipe Moody chart visual audit overlay and PDF export markup provider that stays hidden until explicitly opened, separates overlapped markers, exposes all overlapped element names in tooltip/list evidence, renders a compact Log-Log Moody Chart SVG with Re/Darcy f/epsD/regime metrics for PDF reports, and removes unsupported/unused top-level Pipe Properties fields from the UI
engineering-pump-formula-defense-live-audit.js public-safe live Pump Formula Defense badges, trace-row source/literature notes, protected backend refresh bridge, input-to-output calculation matrix with Required Pump Head and Actual Pump Head separated, backend routeTrace/systemHead discharge-loss fallbacks, margin-criteria-aware summary with canonical OK/Warning/Cavitation Risk labels, and Performance Refresh Governor scheduled open-window realtime refresh for advisor-facing pump NPSH evidence
...
tools/validate-route-trace-default-lock.cjs Node validation for default-hidden canvas route-trace overlays, pump-summary route-trace/vapor-pressure lock, canonical Hydraulic NPSH and Backend Valid status matrix rows, SNK presentation-row lock, SNK Sink Flow/Sink Elev./Sink Head readouts, audit/debug unlock APIs, and cache-busted runtime load
...
tools/validate-pump-status-visual-lock.cjs Node validation for Pump canvas status-badge logic, disconnected-pump Incomplete override, NPSHa-only Warning state, valid Safe state, NPSH Risk state, critical load order, cache key, npm script, and manifest lock
...
tools/validate-dropdown-focus-guard-runtime.cjs Node validation for dropdown focus release before aria-hidden=true, Simulation menu trigger fallback focus, critical cache key order after the protected bundle, npm script, and manifest lock
tools/validate-pdf-export-progress-runtime.cjs Node validation for compact PDF export progress overlay loading, 5-100% milestone contract, PDF-only command binding, XLSX isolation, progress event bridge, no solver/backend/network calls, professional PDF export milestone publishing, cache key, npm script, E2E script, and manifest lock
tools/validate-excel-calculation-trace-runtime.cjs Node validation for Menu -> File -> Export Excel Calculation Trace runtime loading, single Inputs sheet contract, formula-linked OUTPUT PUMP and OUTPUT PFV dashboard blocks, no non-engineering Trace control/toggle rows, application-resolved engineering value reference-baseline parity, cached application-result parity values, Fluid Basis water-property formulas, formula-linked P-h data/chart, suction/discharge PFV formulas, NPSH and required-head formulas, native OpenXML chart injection, Pipe 2 Moody chart coverage, cache key, npm script, E2E script, and manifest lock
tools/validate-excel-calculation-trace-sensitivity.cjs Windows Excel COM validation that exports the workbook, opens it in Excel, runs CalculateFullRebuild, changes 14 major Inputs sheet parameters including Temperature, SRC flow/pressure/elevation, Manual NPSHr, SNK flow/pressure/elevation, suction/discharge pipe length/diameter, and suction/discharge fitting K, and fails if the linked Fluid Basis, PFV, NPSH, or pump discharge outputs do not recalculate
...
tools/validate-src-connect-context-runtime.cjs Node validation for clean SRC context menu actions, native Connect-to-Pipe/Fitting/Valve mode delegation, native Object Properties/Delete Source delegation, capture-phase legacy menu suppression, cache-busted runtime load, and manifest lock; run with `npm run validate:src-connect-context`
...
tools/validate-pipe-properties-cleanup-runtime.cjs Node validation for early Pipe Properties cleanup runtime loading, removed field/label registry, segment z-column and optional hL Allow column cleanup, calculation-phase hooks, hard-hidden CSS, and scroll stability contract
...
tools/validate-pipe-canvas-hydraulic-label.cjs Node validation for Pipe/Fitting/Valve canvas hydraulic label runtime loading, 5-decimal display contract, requested full row labels, cache key, npm script, and manifest lock
...
tools/validate-calculation-lifecycle-runtime.cjs Node validation for unified calculation lifecycle status mapping, Run/Refresh command observation, realtime event subscriptions, simulation-load settle command release, busy watchdogs, load order, cache key, npm script, and audit-safe no-mutation guard
...
tools/validate-simulation-case-6-enabled.cjs Node validation for enabled Simulation Cases 6 menu metadata, P-2941A hot-water sample-file integrity, current-layout migrated .untirta payload, frontend/API mirror byte identity, and optional Papah-sim-6 semantic synchronization
tools/validate-simulation-load-transaction-manager.cjs Node validation for Simulation Load Transaction Manager runtime loading, cache key, index load order after app.bundle and before Open File readiness, session-token API, AbortController cleanup, stale-result guard strings, Validate/Run release fallback, npm script, and manifest lock
tools/validate-runtime-health-audit.cjs Node validation for Runtime Health Audit loading, cache key, load order after transaction manager and performance baseline, settled-load event listeners, bounded history, idle maintenance, load-footprint growth detection, reliability evidence snapshot/panel API, self-healing delegation, npm script, E2E script, and no backend/network/storage/formula mutation guard
tests/e2e/simulation-load-burst-reliability.spec.cjs Playwright stress test for repeated Simulation Case 1/4/6 switching, external `.untirta` File -> Open loading, final canvas readiness, and Validate clickability after every load
tests/e2e/runtime-health-audit.spec.cjs Playwright test for Runtime Health Audit case health recording, idle diagnostic buffer trimming, load-footprint growth detection/transient artifact pruning, opt-in reliability evidence panel rendering, and stale Calculating command self-healing after a settled simulation load
...
tools/upgrade-active-simulation-cases-global-runtime.cjs Targeted migration for enabled Simulation Cases 1, 4, and 6 that synchronizes current Fluid Basis/SRC/SNK aliases, manual NPSHr mode, canonical route/layout, input-only runtime state, and mirrored sample assets
tools/validate-active-simulation-cases-global-runtime.cjs Validation lock for enabled Simulation Cases 1, 4, and 6 covering global calculation authority, canonical route/layout, synchronized input aliases, absence of persisted stale results, and byte-identical mirrored assets
tests/e2e/active-simulation-cases-global-runtime.spec.cjs Browser validation for Simulation Cases 1, 4, and 6 covering current sample metadata, canonical route/layout, synchronized inputs, clickable Validate readiness, protected-backend results, and expired-warning absence
tests/e2e/fluid-basis-phase-chart-visibility.spec.cjs Browser visibility lock for the Fluid Basis Pressure-enthalpy phase chart covering clean initial Water display, immediate removal for Methanol and Custom Fluid, Water restoration, stable task-window visibility state, and absence of page errors
```
...
Bilingual terminology runtime cache key: engineering-bilingual-improvements.js?v=20260711-src-input-flash-lock1
Source canvas parameter runtime cache key: engineering-src-canvas-parameter-runtime.js?v=20260712-route-warning-color-lock1
Source temperature runtime cache key: engineering-source-temperature-runtime.js?v=20260711-src-input-flash-lock1
Source volumetric-only runtime cache key: engineering-source-volumetric-only-runtime.js?v=20260711-src-input-flash-lock1
SRC connect context runtime cache key: engineering-src-connect-context-runtime.js?v=20260706-src-connect-context1
Fluid Basis phase chart runtime cache key: engineering-fluid-basis-phase-chart-runtime.js?v=20260712-fluid-phase-chart-water-only1
...
Pipe canvas hydraulic label runtime cache key: engineering-pipe-canvas-hydraulic-label-runtime-20260716-smooth-drag1.js?v=20260716-canvas-object-smooth-drag1
...
Simulation load transaction manager cache key: engineering-simulation-load-transaction-manager-20260715-export-lock-dedupe1.js?v=20260716-canvas-object-smooth-drag1
Performance baseline runtime cache key: engineering-performance-baseline-runtime.js?v=20260712-performance-console-clean1
Runtime health audit cache key: engineering-runtime-health-audit.js?v=20260710-runtime-health-audit-phase10-1
Performance refresh governor cache key: engineering-performance-refresh-governor.js?v=20260629-live-evidence1
Pump edit fast lane cache key: engineering-pump-edit-fast-lane.js?v=20260710-pump-edit-manual-npshr-local1
Realtime calculation defense cache key: engineering-realtime-calculation-defense.js?v=20260711-src-task-window-flash-lock1
...
Open file readiness gate cache key: engineering-open-file-readiness-gate.js?v=20260716-canvas-object-smooth-drag1
...
Canvas clear/reset guard cache key: engineering-canvas-clear-reset-guard.js?v=20260709-clear-canvas-browser-reload1
...
Pipe Moody chart audit cache key: engineering-pipe-moody-chart-audit.js?v=20260708-pipe-moody-export-chart5
...
Pump warning lifecycle cleanup cache key: engineering-pump-envelope-warning-cleanup-runtime.js?v=20260712-warning-lifecycle-current-request-lock1
Pump formula defense live audit cache key: engineering-pump-formula-defense-live-audit.js?v=20260711-idempotent-refresh-lock1
...
Live parameter repaint lock cache key: engineering-live-parameter-repaint-lock.css?v=20260712-route-warning-color-lock1
...
Route trace/default canvas status validation: npm run validate:route-trace-default-lock
Export canvas snapshot validation: npm run validate:export-canvas-snapshot-lock
Model snapshot export validation: npm run validate:model-snapshot-export
Export equation professional validation: npm run validate:export-equation-professional
Dropdown focus guard validation: npm run validate:dropdown-focus-guard
Dropdown focus guard E2E: npm run test:e2e:dropdown-focus-guard
PDF export progress validation: npm run validate:pdf-export-progress
PDF export progress E2E: npm run test:e2e:pdf-export-progress
...
SNK Object Properties input stability E2E: npm run test:e2e:sink-input-stability
Pump readiness visibility validation: npm run validate:pump-readiness-visibility-lock
Pump status visual lock validation: npm run validate:pump-status-visual-lock
Pump envelope warning cleanup validation: npm run validate:pump-envelope-warning-cleanup
Pump warning lifecycle browser regression: npm run test:e2e:warning-lifecycle
...
Open file readiness gate validation: npm run validate:open-file-readiness-gate
Performance baseline validation: npm run validate:performance-baseline
Performance baseline E2E: npm run test:e2e:performance-baseline
Runtime health audit validation: npm run validate:runtime-health-audit
Runtime health audit E2E: npm run test:e2e:runtime-health-audit
...
Simulation case file compatibility validation: npm run validate:simulation-case-file-compatibility
Simulation case 6 enabled validation: npm run validate:simulation-case-6-enabled
Simulation load transaction manager validation: npm run validate:simulation-load-transaction-manager
Simulation UNTIRTA current-layout validation: npm run validate:simulation-untirta-current-layout
Active simulation cases global-runtime upgrade: npm run upgrade:active-simulation-cases-global-runtime
Active simulation cases global-runtime validation: npm run validate:active-simulation-cases-global-runtime
Active simulation cases global-runtime browser test: npm run test:e2e:active-simulation-cases-global-runtime
