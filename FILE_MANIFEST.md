...
engineering-pump-envelope-warning-cleanup-runtime.js public-safe warning-panel cleanup guard that sanitizes deprecated pump performance/envelope/BEP/design-duty warnings (Design Flow, Design Head, Design Efficiency, BEP Flow, POR/AOR) from pump results and the canvas Warnings panel while preserving hydraulic NPSH warnings and Manual NPSHr validation
...
engineering-sink-input-stability-runtime.js SNK Object Properties numeric-input fast lane that keeps Flow Demand, Reference Pressure, and Elevation user-owned while typing, provides immediate canonical preview, coalesces keystrokes into one protected backend transaction, and prevents competing legacy listeners from replacing a partially typed value
...
engineering-simulation-load-transaction-manager.js public-safe Simulation Load Transaction Manager for Menu -> File -> Open and Simulation Case opens; creates a per-load session token with transaction-owned AbortController signal, aborts stale fetch/body/file reads with AbortController/WeakMap guards, blocks and aborts stale FileReader load events, exposes assertCurrent/guardAsyncResult/signal/setSessionTimeout APIs for async stale-result rejection and abortable delayed work, warms key runtime scripts and selected simulation-case payloads in browser cache, cleans transient warning/route-trace UI before applying the next model, tags backend calls with X-NPSH-Load-Session, releases Validate/Run command locks on load complete/abort/failure/stale rejection, and prevents stale file/case results from leaking into the next active simulation without changing solver formulas
...
engineering-canvas-fast-preview-runtime.js public-safe canvas fast-preview bridge that repaints Pump, Pipe/Fitting/Valve, and SNK live readouts on the next animation frame after input/dependency changes, uses current Fluid Basis properties and existing calculation traces, keeps disconnected pumps Incomplete/Unverified, normalizes Hydraulic NPSH and Backend Valid status labels during transient previews, updates Required Head only, and keeps the protected backend solve authoritative without calling updateSimulation
...
Canvas fast preview runtime cache key: engineering-canvas-fast-preview-runtime.js?v=20260710-canvas-fast-preview21
...
Route audit cache key: engineering-route-trace-audit-20260704-sink-pabs-dedupe1.js?v=20260711-sink-input-stability1
Sink input stability runtime cache key: engineering-sink-input-stability-runtime.js?v=20260711-sink-input-stability1
...
Simulation load transaction manager cache key: engineering-simulation-load-transaction-manager.js?v=20260711-simulation-load-visual-wrapper-lock1
...
Pump envelope warning cleanup cache key: engineering-pump-envelope-warning-cleanup-runtime.js?v=20260711-pump-warning-wrapper-lock1
