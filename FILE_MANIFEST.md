...
engineering-realtime-calculation-defense.js public-safe realtime stale/calculating/freshness bridge that marks calculation results stale immediately when object-property inputs change, autosolves trusted user edits through the protected backend as the primary calculation path with 240 ms input and 90 ms change debounce, carries sequence/request transaction metadata, records initial/final dependency fingerprints, emits superseded events without letting stale backend results overwrite newer input, exposes Failed when backend recalculation fails while keeping the last valid result visible, suppresses bootstrap/sample-menu Calculating status until explicit user calculation intent, treats Simulation Case browsing as menu-only, avoids heavy chart/linked-view refresh during typing, debounces linked-view refresh through the Performance Refresh Governor after backend completion, and refreshes linked report/task-window views from current backend results
engineering-calculation-lifecycle-runtime.js public-safe lifecycle status bridge that unifies input-changed, preparing, waiting-debounce, calculating, applying-results, refreshing-evidence, current, and failed states from realtime calculation events and Validate/Refresh/Open Sample Case commands, separates menu-browse/sample-open/manual-solve modes, treats the manual command as Validate / Refresh Evidence while realtime autosolve remains primary, disables Run/Validate commands while calculation/evidence refresh is busy, suppresses bootstrap calculating/applying and orphan evidence-refresh events while sample-case selection is idle, permits Refreshing evidence only for manual evidence refresh, and never mutates calculation data
engineering-calculation-progress-overlay.js public-safe non-blocking compact calculation progress overlay that listens to lifecycle status, realtime calculation events, and Validate/Refresh/Open Sample Case/Simulation Case browse intent, shows only Reading inputs for menu browsing, shows Reading/Solving/Updating for sample-open, reserves Refreshing evidence for manual evidence refresh, auto-hides on Current/autosolve complete/fallback evidence completion, shows short failed-calculation fallback text, and never mutates model/results/traces/charts
...
Realtime calculation defense cache key: engineering-realtime-calculation-defense.js?v=20260617-realtime-final-state1
Calculation lifecycle runtime cache key: engineering-calculation-lifecycle-runtime.js?v=20260617-calculation-lifecycle-final-state1
Calculation progress overlay cache key: engineering-calculation-progress-overlay.js?v=20260617-calculation-progress-realtime1
